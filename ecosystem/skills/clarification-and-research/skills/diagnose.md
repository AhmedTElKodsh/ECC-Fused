...
> Generated mirror. Canonical source: `skills/04-Testing-and-QA/diagnose/SKILL.md`.
...
Provide the Diagnose workflow in the ECC-Fusion routed system.

## Trigger

Use when the active command or route calls for `diagnose`.
...
1. Reproduce or precisely describe the failure surface.
2. Form hypotheses and test them one at a time.
3. Keep evidence separate from guesses.
...
`diagnose` should produce a compact result with route, rationale, blockers, and next command.
