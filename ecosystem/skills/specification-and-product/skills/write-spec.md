...
> Generated mirror. Canonical source: `skills/01-Discovery/write-spec/SKILL.md`.
...
Provide the Write Spec workflow in the ECC-Fusion routed system.

## Trigger

Use when the active command or route calls for `write-spec`.
...
1. **Ask One Question at a Time:** Do NOT dump a 20-point checklist on the user. Start with a single, high-level question (e.g., "What is the primary problem we are solving?").
2. **Listen and Expand:** Use the user's answers to draft the spec (goals, non-goals, scope, AC IDs).
3. **Draft the Artifact:** Write to `.planning/REQUIREMENTS.md` incrementally.
...
5. **Block when Contradictory:** Stop and ask for clarification if product intent is contradictory.
...
`write-spec` should produce a compact result with route, rationale, blockers, and next command.
