...
Provide the Grill With Context workflow in the ECC-Fusion routed system.

## Trigger

Use when the active command or route calls for `grill-with-context`.
...
1. Challenge the plan against users, constraints, edge cases, and failure modes.
2. Use existing repo context before asking broad questions.
3. Output tightened requirements and remaining risks.
...
`grill-with-context` should produce a compact result with route, rationale, blockers, and next command.
